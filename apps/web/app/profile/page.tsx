        ...
        <div className="min-h-screen bg-gray-50">
            ...
            <header className="sticky top-0 z-40 border-b border-gray-200 bg-white/90 backdrop-blur">
                <div className="mx-auto flex max-w-3xl items-center justify-between px-6 py-3">
                    ...
                    <Link href="/dashboard" className="flex items-center gap-1.5 text-xs font-medium text-gray-500 hover:text-gray-700 transition">
            ...
            <div className="max-w-3xl mx-auto px-6 py-10 space-y-6">
                ...
                {isLoading ? (
                    ...
                    <div className="space-y-6">
                        <AvatarSection me={me!} queryClient={queryClient} fileInputRef={fileInputRef} />
...
function AvatarSection({
    ...
    const mutation = useMutation({
        ...
        onError: () => setError("Upload failed. Make sure Cloudinary is configured."),
    });
            ...
            <div className="flex items-center gap-5">
                <div className="relative shrink-0">
                    {avatarSrc ? (
                        ...
                        <div className="h-18 w-18 flex items-center justify-center rounded-full bg-indigo-600 text-2xl font-bold text-white" style={{ width: 72, height: 72 }}>
    ...
    return (
        <div className="rounded-xl border border-gray-200 bg-white p-6">
            ...
            <div className="space-y-4 max-w-sm">
                ...
                {success && <p className="text-xs text-green-600">Name updated successfully.</p>}
                <button
                    onClick={() => mutation.mutate()}
                    disabled={mutation.isPending || name === me.name || !name.trim()}
...
/* ── Password ─────────────────────────────────────────── */
function PasswordSection({ queryClient }: { queryClient: ReturnType<typeof useQueryClient> }) {
    const [form, setForm] = useState({ currentPassword: "", newPassword: "", confirmPassword: "" });
    const [success, setSuccess] = useState(false);
    const [error, setError] = useState("");

    const mutation = useMutation({
        mutationFn: () => updateProfile({ currentPassword: form.currentPassword, newPassword: form.newPassword }),
        onSuccess: () => {
    ...
    return (
        <div className="rounded-xl border border-gray-200 bg-white p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 max-w-sm">
                {(["currentPassword", "newPassword", "confirmPassword"] as const).map((field) => (
                    <div key={field}>
                        ...
                        <input
                            type="password"
                            value={form[field]}
                            onChange={(e) => { setForm((f) => ({ ...f, [field]: e.target.value })); setError(""); setSuccess(false); }}
                            className="w-full rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20"
                        />
                    </div>
                ))}
                ...
                <button
                    type="submit"
                    disabled={mutation.isPending || !form.currentPassword || !form.newPassword || !form.confirmPassword}
                    className="rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed"
                >
...
function DangerSection({ router }: { router: ReturnType<typeof useRouter> }) {
    ...
    return (
        <div className="rounded-xl border border-red-200 bg-white p-6">
            ...
            <p className="text-xs text-gray-500 mb-4">You will be returned to the login page.</p>
            <button
